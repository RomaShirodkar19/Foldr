...
public class ClerkJwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            String headerJson = new String(Base64.getDecoder().decode(chunks[0])); // Decode header
            ObjectMapper objectMapper = new ObjectMapper();
            JsonNode headerNode = objectMapper.readTree(headerJson);

            if(!headerNode.has("kid")) {
                response.sendError(HttpServletResponse.SC_FORBIDDEN, "JWT token missing 'kid' in header");
                return;
            }

            String kid = headerNode.get("kid").asText();
            PublicKey publicKey = jwksProvider.getPublicKey(kid);// This will throw an exception if the key is not found or invalid
